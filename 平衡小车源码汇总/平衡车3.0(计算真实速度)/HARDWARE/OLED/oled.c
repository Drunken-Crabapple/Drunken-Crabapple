...
void OLED_ShowVelocity(float velocity)
{
	int ge,shi,bai,qian;
	int vel;
	OLED_ShowStr(0,5,"V:",2);
	OLED_ShowStr(70,5,"m/s",2);
	vel = (int)(velocity*1000);
	...
	if(vel>0)
	{
		...
		OLED_ShowSNum(24,5,qian,2);
		OLED_ShowStr(32,5,".",2);
		OLED_ShowSNum(40,5,bai,2);
		OLED_ShowSNum(48,5,shi,2);
		OLED_ShowSNum(56,5,ge,2);
	}
	else
	{
  	OLED_ShowStr(16,5,"-",2);
...
{
	int Roll,Yaw;
	OLED_ShowStr(0,1,"Roll:",2);
	OLED_ShowStr(0,3,"Yaw:",2);
	Roll=(int)(roll*10);
	Yaw=(int)(yaw*10);
	//对浮点数做处理，于是可以显示在lcd上
	...
	if(Roll<0)
	{
		OLED_ShowStr(40,1,"-",2);		//显示负号
		Roll=-Roll;	
	}
	else OLED_ShowStr(40,1," ",2); //掩盖负号
	OLED_Shownum3(48,1,Roll,2);	  //显示翻滚角
	...
	else OLED_ShowStr(32,3," ",2); 
	OLED_Shownum3(40,3,Yaw,2);	    //显示航向角
}
