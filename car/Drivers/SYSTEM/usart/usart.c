...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
    ...
    if(huart->Instance == USART2)
    {
        if((openmv_rx_sta & 0x8000) == 0)
        {
             if(openmv_rx_sta & 0x4000)
             {
                if(openmv_rx_buffer[0] != 0x0a)
                {
                    openmv_rx_sta = 0;
                }
                else
                {
                    openmv_rx_sta |= 0x8000;
                }
             }
             else
             {
                if(openmv_rx_buffer[0] == 0x0d)
                {
                    openmv_rx_sta |= 0x4000;
                }
                else
                {
                    openmv_rx_buf[openmv_rx_sta & 0x3fff] = openmv_rx_buffer[0];
                    openmv_rx_sta++;

                    if((openmv_rx_sta & 0x3fff) > (OPENMV_REC_LEN - 1))
                    {
                        openmv_rx_sta = 0;
                    }
                }
...
{
    HAL_UART_IRQHandler(&g_uart2_openmv_handle);
}
